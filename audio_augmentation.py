...
class RandomAudioAugmentation:
    ...
    def __init__(self, num_aug_per_sample=1, aug_dict={}, clip_range=[0.85, 1.0], sample_rate=16000):
        self.sample_rate = sample_rate
        self.num_aug_per_sample = num_aug_per_sample
        self.aug_dict = aug_dict
        self.aug_names = list(self.aug_dict.keys())
        self.clip_range = clip_range
        self.aug_object = AudioPerturbations(sample_rate=self.sample_rate)

    def custom_clip(self, audio_data):
        audio_length = audio_data.shape[0]
        clip_frac = random.uniform(*clip_range)
        assert(clip_frac <= 1)
    ...
    def __call__(self, audio_data):
        ...
        perturbed = perturber.apply_perturbation(audio_data, aug_name, **aug_param)
...
class AudioPerturbations:
    ...
    def simulate_mp3_compression(
        ...
        quantized_magnitude = np.round(magnitude / max_mag * quantization_levels) / quantization_levels * max_mag
    ...
    def apply_equalization(
        self, 
        audio: np.ndarray, 
        eq_gains_db: Optional[np.ndarray] = None
    ) -> np.ndarray:
        """
        ...
        for i, (freq, gain_db) in enumerate(zip(bands, eq_gains_db)):
            if i == 0:  # Low-pass for first band
                ...
                sos = signal.butter(2, [bands[i-1], freq], btype='band', fs=self.sample_rate, output='sos')
    ...
    def get_all_perturbations(self) -> Dict[str, Dict[str, Any]]:
        ...
        return {
            ...
            "reverberation": {"room_scale": [0.3, 0.5, 0.8], "damping": [0.3, 0.5, 0.8]},
            "mp3_compression": {"bitrate_kbps": [8, 16, 24, 32, 40]},
            
            # Additional perturbations
            "jittering": {"max_shift": [1, 2, 3], "probability": [0.05, 0.1, 0.2]},
            "dynamic_compression": {"ratio": [2.0, 4.0, 8.0], "threshold": [-30, -20, -10]},
    ...
    def apply_perturbation(
        ...
        **kwargs
        ...
        perturbation_map = {
            ...
            "background_noise_pink": lambda x, **k: self.add_background_noise(x, noise_type="pink", **k),
            "background_noise_white": lambda x, **k: self.add_background_noise(x, noise_type="white", **k),
...
if __name__ == "__main__":
    debug_audio = "/home/tst000/projects/LatentGroot/test/dummy_dataset/spk_1246_txt_69_ref_3.wav"
    sample_rate = 16000
